...
## In flight / parked
- **AUTH — DONE (July 31, 2026)**: Google-only sign-in merged to `main` (commits `01b4618`, merge `9ac9f99`). Apple removed entirely (owner descoped it). JWT sessions, zero DB, env-gated (`AUTH_SECRET` + `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET`) — production still has none of these set, so the live site is unchanged: nav has no "Sign in", `/account` shows a coming-soon explainer, `/api/auth/*` answers 404. Verified `npm run build` clean both env-less and with dummy vars, `npm run lint` clean, live-checked puptv.vercel.app after deploy (`/`, `/create`, `/account` all 200 and correct, `/api/health` → `{"live":true}`). New: `/account` page, `src/components/AccountMenu.tsx` (shared nav sign-in/avatar+dropdown), `docs/auth-setup.md`. **Chad's remaining step**: create the Google OAuth client (~10 min, exact click-path in `docs/auth-setup.md`) and set the three env vars in Vercel to flip sign-in on. Strategic note unchanged: this same Google OAuth app later gets the `youtube.upload` scope — one-line change flagged in `src/lib/auth.ts`, full verification process in `docs/youtube-verification.md`.
